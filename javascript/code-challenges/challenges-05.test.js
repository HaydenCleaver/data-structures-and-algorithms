...
const toLastNames = people => {
  ...
  return newArray = people.map((val) => val.firstName + ' ' + val.lastName);
};
...
const addValues = (arr) => {
  let total = arr.reduce((newVal, num) => {
    return newVal + num;
  }, 0);
...
const addPurchases = (arr) => {
  let total = arr.reduce((newVal, val) => {
    return newVal + val.purchasePrice;
  }, 0);
...
const countNumberOfElements = (arr) => {
  let count = arr.reduce((countVal, ele) => {
    return countVal + 1;
  }, 0);
...
const returnNames = (arr) => {
  let names = arr.reduce((newArray, obj) => {
...
const reversedString = (str) => {
  let arr = str.split('');
  let revString = arr.reduce((newString, oldStr) => {
    return oldStr + newString;
  }, '');
  return revString;
};
...
const countNumberOfChildren = (arr) => {
  let childCount = arr.reduce((count, obj) => {
    return (obj.children ? count + obj.children.length : count + 0);
  }, 0);
  return childCount;
};
...
const extractStat = (statName, arr) => {
  let match = arr.reduce((search, val)=> {
    return (statName === val.stats.name ? search = val.stats.name : search = null);
  }, '');
