...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  let newArray = (arr.filter(el => forbiddenValues.indexOf(el) === -1));
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let statArray = (arr.filter(stat => stat.baseStat > minBaseStat));
...
const getStatName = (arr, minBaseStat) => {
  ...
  let statArray = (arr.filter(obj => obj.baseStat > minBaseStat).map(val => val.name));
